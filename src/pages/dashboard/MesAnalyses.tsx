  ...
  return (
    <>
      ...
      <div className="cpl-desktop" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderBottom: isLast ? 'none' : '0.5px solid #f0f4f7', background: selected ? '#f0f7fb' : 'transparent', cursor: selectionMode ? 'pointer' : 'default', transition: 'background 0.15s' }}
        ...
        {a.status === 'processing' ? <div style={{ width: 44, height: 44, borderRadius: '50%', border: '2.5px solid #edf2f7', borderTopColor: C.teal, animation: 'spin 0.9s linear infinite', flexShrink: 0 }} /> : a.score != null ? <ScoreRing score={a.score} size={44} /> : null}
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
            {a.status === 'processing' ? <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 700, color: C.teal }}><span style={{ width: 6, height: 6, borderRadius: '50%', background: C.teal, animation: 'pulse 1.5s ease-in-out infinite' }} />{(a as Analyse & { progress_message?: string }).progress_message || 'Analyse en cours…'}</span> : <>
        ...
        {!selectionMode && a.status !== 'processing' && (
          <div style={{ display: 'flex', gap: 5, alignItems: 'center', flexShrink: 0 }} onClick={e => e.stopPropagation()}>
            {!a.is_preview && a.status === 'completed' && <button onClick={() => onShare(a.id, title)} style={{ padding: '6px 13px', borderRadius: 7, background: '#f8fafc', border: '1px solid #edf2f7', color: '#64748b', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, whiteSpace: 'nowrap' }}><Share2 size={11} /> Partager</button>}
            <Link to={`/dashboard/rapport?id=${a.id}`} style={{ padding: '6px 13px', borderRadius: 7, background: '#2a7d9c', color: '#fff', fontSize: 12, fontWeight: 700, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4, whiteSpace: 'nowrap' }}><ExternalLink size={11} /> Rapport</Link>
      ...
      <div className="cpl-mobile" style={{ display: 'none', flexDirection: 'column', padding: 14, borderBottom: isLast ? 'none' : '0.5px solid #f0f4f7' }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 10 }}>
          {a.status === 'processing' ? <div style={{ width: 40, height: 40, borderRadius: '50%', border: '2.5px solid #edf2f7', borderTopColor: C.teal, animation: 'spin 0.9s linear infinite', flexShrink: 0 }} /> : a.score != null ? <ScoreRing score={a.score} size={40} /> : null}
          <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {a.status !== 'processing' && <div style={{ display: 'flex', gap: 6 }}>
          {!a.is_preview && a.status === 'completed' && <button onClick={() => onShare(a.id, title)} style={{ padding: '7px 12px', borderRadius: 7, background: '#f8fafc', border: '1px solid #edf2f7', color: '#64748b', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}><Share2 size={11} /> Partager</button>}
          <Link to={`/dashboard/rapport?id=${a.id}`} style={{ flex: 1, padding: '7px 0', borderRadius: 7, background: '#2a7d9c', color: '#fff', fontSize: 12, fontWeight: 700, textDecoration: 'none', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}><ExternalLink size={11} /> Rapport</Link>
...
function SimpleRow({ a, onDelete, isLast, selectionMode, selected, onToggleSelect, onShare }: { a: Analyse; onDelete: (id: string) => void; isLast: boolean; selectionMode: boolean; selected: boolean; onToggleSelect: (id: string) => void; onShare: (id: string, title: string) => void; }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 16px', borderBottom: isLast ? 'none' : '0.5px solid #f0f4f7', background: selected ? '#f0f7fb' : 'transparent', cursor: selectionMode ? 'pointer' : 'default', transition: 'background 0.15s' }}
      ...
      : <div style={{ display: 'flex', gap: 5, alignItems: 'center', flexShrink: 0 }} onClick={e => e.stopPropagation()}>
          {!a.is_preview && a.status === 'completed' && <button onClick={() => onShare(a.id, title)} style={{ padding: '5px 11px', borderRadius: 7, background: '#f8fafc', border: '1px solid #edf2f7', color: '#64748b', fontSize: 11, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, whiteSpace: 'nowrap' }}><Share2 size={10} /> Partager</button>}
          <Link to={`/rapport?id=${a.id}`} style={{ padding: '5px 11px', borderRadius: 7, background: C.simple, color: '#fff', fontSize: 11, fontWeight: 700, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 3, whiteSpace: 'nowrap' }}><ExternalLink size={10} /> Rapport</Link>
