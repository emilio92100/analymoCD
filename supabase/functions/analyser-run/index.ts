...
function buildDocumentPrompt(p: string): string {
  ...
  parts.push('- travaux_en_cours : extraire les travaux VOTES en AG mais pas encore réalisés (section "TRAVAUX votés en cours" ou équivalent). Pour chaque travail : label (sans montant), date_ag (date du vote), montant, entreprise si mentionnée. Ces travaux sont critiques pour l acheteur car ils génèrent des appels de fonds futurs. Mettre dans points_vigilance si montant significatif (>5000€).');
  ...
  parts.push('- infos_complementaires : tout ce qui ne rentre pas dans les champs prévus mais qui est utile pour un acheteur (nombre de bâtiments, appels de provisions annuels, fibre optique, patrimoine syndicat, AFUL, syndicat secondaire...). Toujours commencer le label par un émoji adapté au contenu : 🏗 pour bâtiments, 📅 pour périodicité, 💾 pour fibre/réseau, 🏛 pour statut juridique, 🔢 pour chiffres divers, 📋 pour administratif, etc.');
