...
- RÈGLE HISTORIQUE CHARGES N-1 / N-2 : si un pre-etat date ou etat date est fourni, TOUJOURS extraire pre_etat_date.historique_charges avec deux entrees (N-1 et N-2) — chaque entree avec exercice, annee, budget_appele, charges_reelles. Ces donnees figurent systematiquement dans les pre-etats dates (section "charges sur les deux derniers exercices clos"). Si seulement N-1 disponible, remplir la premiere entree et laisser la seconde avec annee=null. Ne JAMAIS omettre ce tableau — il est critique pour l acheteur pour comparer ecart budget vote / charges reelles.
