...
function SidebarPro({ subscription, proCredits, onClose, unreadTickets, creditsLoading, agenceRole }: { subscription: ProSubscription | null; proCredits: ProCredits | null; onClose?: () => void; unreadTickets?: number; creditsLoading?: boolean; agenceRole?: string | null }) {
  ...
  // Le pro peut basculer entre les deux via le bouton soleil/lune en bas de la sidebar.
  // Par défaut : clair. Le choix est gardé entre les sessions sur ce navigateur.
  ...
  const TEXT_ACTIVE = isDark ? '#ffffff' : '#0f2d3d';
  const MUTED = isDark ? 'rgba(255,255,255,0.45)' : '#94a3b8';
  ...
  const Skeleton = ({ width = 24, height = 14 }: { width?: number; height?: number }) => (
    <span style={{
      ...
      background: isDark
        ...
        : 'linear-gradient(90deg, rgba(15,45,61,0.06), rgba(15,45,61,0.12), rgba(15,45,61,0.06))',
    ...
    <aside data-sidebar-pro data-theme-mode={isDark ? 'dark' : 'light'} style={{ width: 260, minHeight: '100vh', height: '100%', background: BG, display: 'flex', flexDirection: 'column', borderRight: isDark ? 'none' : '1px solid #e2e8f0', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ margin: '0 14px 8px' }}>
        {creditsLoading && !subscription ? (
          ...
          <div style={{ padding: '12px 14px', borderRadius: 11, background: isDark ? 'linear-gradient(135deg, rgba(22,163,74,0.4), rgba(21,128,61,0.4))' : 'rgba(255,255,255,0.7)', border: isDark ? '1px solid rgba(134,239,172,0.25)' : '1px solid #e2e8f0' }}>
          ...
          // ── État 4 : Aucun abonnement — adapté selon le mode ──
          <Link to="/dashboard/abonnement" onClick={onClose}
            style={{
              display: 'block', padding: '12px 14px', borderRadius: 11,
              background: isDark ? 'linear-gradient(135deg, rgba(125,211,252,0.12), rgba(125,211,252,0.05))' : 'rgba(255,255,255,0.7)',
              border: isDark ? '1.5px dashed rgba(125,211,252,0.4)' : '1.5px dashed #7dd3fc',
...
function TopbarPro({ onMenuClick, title, mobileTitle, proProfile, unreadCount, notifications, onMarkAllRead, onClickNotification }: {
  ...
  // 🆕 `link` : notification qui ne pointe pas vers un rapport (message du support).
      ...
      <AnimatePresence>
        {cancelStep === 1 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.5)', padding: 12 }}
      ...
      <AnimatePresence>
        {cancelStep === 2 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              onClick={e => e.stopPropagation()}
              className="cancel-reasons-modal"
              style={{ background: '#fff', borderRadius: 20, padding: '24px 22px', maxWidth: 440, width: '100%', textAlign: 'center', boxShadow: '0 24px 64px rgba(0,0,0,0.2)', maxHeight: '90vh', overflowY: 'auto' }}>
                      ...
                      <div style={{ padding: '18px 24px 22px' }}>
                        ...
                        {upgradePreview.is_downgrade && (
                          <div style={{ background: '#fef3c7', border: '1px solid #fcd34d', borderRadius: 10, padding: '12px 14px', marginBottom: 14 }}>
                            ...
                            <div style={{ borderTop: '1px solid #fcd34d', marginTop: 10, paddingTop: 9 }}>
                              <p style={{ fontSize: 11, color: '#92400e', margin: 0, lineHeight: 1.5, opacity: 0.85 }}>
...
export default function DashboardProPage() {
  ...
  type ProNotification = { id: string; analysisId: string; title: string; createdAt: string; read: boolean; link?: string };
