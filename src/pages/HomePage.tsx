...
function SectionTitle({ label, title, accent, sub, saut }: { label: string; title: string; accent: string; sub?: string; saut?: boolean }) {
  const ref = useRef<HTMLDivElement>(null);
      ...
      <div ref={ref} className="text-center mb-8 md:mb-12 px-2">
        ...
        <h2 style={{ ...base, transitionDelay: '0.05s' }} className="text-[clamp(26px,4vw,52px)] font-black tracking-[-0.03em] leading-[1.1] text-[#0f172a] mb-4">
          ...
          <span className="relative inline-block max-w-fit">
            ...
            <span className="absolute -bottom-1 left-0 right-0 h-[4px] bg-[#2a7d9c]/25 rounded-full block" />
          </span>
        </h2>
...
function HeroSection() {
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden px-5 sm:px-10 lg:px-20 pt-16 pb-12" style={{ background: 'linear-gradient(165deg, #ecf4f7 0%, #d8eaef 45%, #c4e0e8 100%)' }}>
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 80, background: 'linear-gradient(180deg, transparent 0%, #ffffff 100%)', pointerEvents: 'none', zIndex: 5 }} />
    </section>
  );
}
...
function PhoneMockup() {
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: 340, height: 580 }}>
      ...
      {/* Badge sécurité — déplacé à droite (la mascotte occupe la gauche) */}
      <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.6 }}
        style={{ animation: _lowPerf ? undefined : "floatA 4.5s ease-in-out 1.6s infinite", position: 'absolute', right: -128, top: '40%', zIndex: 30 }}
...
function HowItWorksSection() {
  ...
  const steps = [
    ...
    },
    {
      ...
      desc: "Faites une offre, négociez ou laissez tomber — en connaissance de cause.",
      descDesktop: "Faites une offre, négociez ou laissez tomber.\nEn connaissance de cause.",
    },
  ];
          ...
          <div className="relative flex justify-between items-start">
            ...
            {steps.map((s, i) => (
              <motion.div key={i}
                ...
                {/* Description */}
                <p className="text-sm text-slate-500 leading-relaxed whitespace-pre-line max-w-[260px]">{s.descDesktop}</p>
              </motion.div>
            ))}
          </div>
        </div>
