...
export default function AuthCallbackPage() {
  ...
  useEffect(() => {
    const progressInterval = setInterval(() => {
      setProgress(p => { if (p >= 95) return p; return p + 1.5; });
    }, 90);
    ...
    const handleCallback = async () => {
      ...
      // 1. Session déjà établie (Google PKCE)
      const { data: existing } = await supabase.auth.getSession();
      if (existing.session) {
        sessionOk = true;
      }

      // 2. token_hash (confirmation email Supabase)
      if (!sessionOk && tokenHash && type) {
        try {
          ...
          if (!error && data.session) sessionOk = true;
        } catch { /* silencieux */ }
      }
      ...
      // 4. access_token dans le hash
      if (!sessionOk && hash && hash.includes('access_token')) {
        try {
      ...
      if (sessionOk) {
        ...
        // Aucune session trouvée — l'email est peut-être déjà confirmé
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(160deg, #f5f9fb 0%, #eaf4f8 100%)', padding: 24, fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      <div style={{ maxWidth: 480, width: '100%', textAlign: 'center' as const }}>
        ...
        <img src="/logo.png" alt="Verimo" style={{ height: 44, objectFit: 'contain' as const, display: 'block', margin: '0 auto 48px' }} />
        ...
        {status === 'already_confirmed' && (
          <div style={{ animation: 'fadeInUp 0.6s ease' }}>
            ...
            <h1 style={{ fontSize: 28, fontWeight: 900, color: '#0f2d3d', marginBottom: 12 }}>Compte activé ! ✅</h1>
            <p style={{ fontSize: 15, color: '#64748b', lineHeight: 1.7, marginBottom: 36 }}>
              Votre adresse email a été confirmée. Connectez-vous pour accéder à votre espace.
            </p>
        ...
        {status === 'error' && (
          <div style={{ animation: 'fadeInUp 0.6s ease' }}>
            ...
            <h1 style={{ fontSize: 28, fontWeight: 900, color: '#0f2d3d', marginBottom: 12 }}>Lien expiré</h1>
            <p style={{ fontSize: 15, color: '#64748b', lineHeight: 1.7, marginBottom: 36 }}>Votre lien a expiré (valable 24h). Veuillez vous réinscrire.</p>
