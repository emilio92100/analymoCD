  ...
  return (
    <div style={{ background: '#fff', color: '#0f172a', overflow: 'hidden', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      {/* ════════════════════════════════════════════
          ...
          Le rapport sert d'argument objectif quand l'acheteur baisse le prix.
          Version épurée : un seul message, un mockup simple (1 bulle / 1 carte / 1 bulle).
            ...
            <Reveal>
              <div className="scenario-visual scenario-4" style={{ position: 'relative' as const, height: 460, background: 'linear-gradient(135deg, #fffbeb, #fef3c7)', borderRadius: 24, padding: 24, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Confetti items={[
                  ...
                  { top: '24%', right: '10%', size: 12, color: VERIMO_CONFETTI_COLORS.green, shape: 'square', delay: 0.5 },
                ...
                {/* Mockup simple : bulle acheteur → carte rapport → bulle agent */}
                <div style={{ width: '100%', maxWidth: 340, display: 'flex', flexDirection: 'column' as const, gap: 12, position: 'relative' as const, zIndex: 2 }}>

                  {/* Bulle acheteur — offre à la baisse */}
                  <div style={{ alignSelf: 'flex-start', maxWidth: '80%', background: '#fff', padding: '11px 14px', borderRadius: '14px 14px 14px 4px', boxShadow: '0 3px 10px rgba(15,45,61,0.06)', fontSize: 13, lineHeight: 1.45, color: '#1f2937' }}>
                    Je propose <strong>−25 000 €</strong>. Le bien a sûrement des défauts.
                  </div>

                  {/* Carte rapport Verimo — sobre */}
                  <motion.div
                    initial={{ opacity: 0, scale: 0.96 }}
                    whileInView={{ opacity: 1, scale: 1 }}
                    viewport={{ once: true, margin: '-60px' }}
                    transition={{ duration: 0.45, delay: 0.3 }}
                    style={{ alignSelf: 'center', width: '100%', background: '#fff', borderRadius: 14, padding: '14px 16px', border: '1px solid #e2e8f0', boxShadow: '0 8px 22px rgba(15,45,61,0.08)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                      <div style={{ fontSize: 10.5, fontWeight: 800, color: '#64748b', letterSpacing: '0.06em' }}>RAPPORT VERIMO</div>
                      <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 4, padding: '3px 9px', background: 'rgba(16,185,129,0.12)', borderRadius: 100, fontSize: 11.5, fontWeight: 800, color: '#047857' }}>
                        <Star size={10} fill="#10b981" stroke="#10b981" /> 17/20
                      </div>
                    </div>
                    <div style={{ height: 6, borderRadius: 3, background: 'linear-gradient(to right, #ef4444 0%, #f59e0b 33%, #fbbf24 50%, #84cc16 75%, #16a34a 100%)', position: 'relative' as const, marginBottom: 10 }}>
                      <div style={{ position: 'absolute' as const, left: '85%', top: -3, width: 12, height: 12, borderRadius: '50%', background: '#fff', border: '2px solid #16a34a', boxShadow: '0 2px 4px rgba(0,0,0,0.15)' }} />
                    </div>
                    <div style={{ fontSize: 12, color: '#475569', lineHeight: 1.5 }}>
                      DPE C · Aucun travaux voté · Copro saine
                    </div>
                  </motion.div>

                  {/* Bulle agent — réponse appuyée sur les chiffres */}
                  <motion.div
