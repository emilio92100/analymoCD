...
export default function AuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    const handleCallback = async () => {
      // 1. Essayer de récupérer le token depuis le hash (cas standard)
      ...
      // 2. Essayer depuis les query params (cas iOS Safari qui strip le hash)
      ...
      // 3. Vérifier si une session existe déjà (l'email a été confirmé mais
      //    iOS a redirigé sans token visible — le compte est quand même activé)
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(160deg, #f5f9fb 0%, #eaf4f8 100%)', padding: 24 }}>
      <div style={{ maxWidth: 480, width: '100%', textAlign: 'center' }}>
        ...
        {/* Cas iOS : compte activé mais pas de session récupérée */}
        {status === 'already_confirmed' && (
          <>
            ...
            <h1 style={{ fontSize: 24, fontWeight: 800, color: 'var(--brand-navy)', marginBottom: 12 }}>Compte activé ! ✅</h1>
            <p style={{ fontSize: 15, color: 'var(--text-secondary)', marginBottom: 32 }}>
              Votre adresse email a bien été confirmée. Vous pouvez maintenant vous connecter à votre espace.
            </p>
            <a href="/connexion" style={{ display: 'inline-block', padding: '14px 32px', borderRadius: 12, fontSize: 15, fontWeight: 700, color: '#fff', background: 'linear-gradient(135deg, var(--brand-teal) 0%, var(--brand-navy) 100%)', textDecoration: 'none', boxShadow: '0 6px 24px rgba(42,125,156,0.3)' }}>
              Se connecter →
            </a>
        ...
        {status === 'error' && (
          <>
            ...
            <p style={{ fontSize: 15, color: 'var(--text-secondary)', marginBottom: 32 }}>Votre lien d'activation a expiré (valable 24h). Veuillez vous réinscrire pour générer un nouveau lien.</p>
